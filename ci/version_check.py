"""Ensure that the release tag matches the arius version number.
...
def get_existing_release_tags():
    ...
    response = requests.get('https://api.github.com/repos/ariustechnology/arius/releases', headers=headers)
...
def check_version_number(version_string, allow_duplicate=False):
    ...
    Returns True if the provided version is the 'newest' arius release
    """
...
if __name__ == '__main__':
    ...
    version_file = os.path.join(here, '..', 'arius', 'arius', 'version.py')
    ...
    with open(version_file, 'r') as f:
        ...
        # Extract the arius software version
        results = re.findall(r'ARIUS_SW_VERSION = "(.*)"', text)

        if len(results) != 1:
            print(f"Could not find ARIUS_SW_VERSION in {version_file}")
            sys.exit(1)

        version = results[0]

    print(f"arius Version: '{version}'")
    ...
    with open(os.getenv('GITHUB_ENV'), 'a') as env_file:
        ...
        tags = ",".join([f"arius/arius:{tag}" for tag in docker_tags])
