...
from .models import (Part, PartCategory, PartCategoryStar, PartRelated,
                     ...
                     rename_part_image)
from .templatetags import arius_extras


class TemplateTagTest(AriusTestCase):
    """Tests for the custom template tag code."""

    def test_define(self):
        """Test the 'define' template tag"""
        self.assertEqual(int(arius_extras.define(3)), 3)

    def test_str2bool(self):
        """Various test for the str2bool template tag"""
        self.assertEqual(int(arius_extras.str2bool('true')), True)
        self.assertEqual(int(arius_extras.str2bool('yes')), True)
        self.assertEqual(int(arius_extras.str2bool('none')), False)
        self.assertEqual(int(arius_extras.str2bool('off')), False)

    def test_add(self):
        """Test that the 'add"""
        self.assertEqual(int(arius_extras.add(3, 5)), 8)

    def test_plugins_enabled(self):
        """Test the plugins_enabled tag"""
        self.assertEqual(arius_extras.plugins_enabled(), True)

    def test_arius_instance_name(self):
        """Test the 'instance name' setting"""
        self.assertEqual(arius_extras.arius_instance_name(), 'arius')

    def test_arius_base_url(self):
        """Test that the base URL tag returns correctly"""
        self.assertEqual(arius_extras.arius_base_url(), '')

    def test_arius_is_release(self):
        """Test that the release version check functions as expected"""
        self.assertEqual(arius_extras.arius_is_release(), not version.isAriusDevelopmentVersion())

    def test_arius_docs_version(self):
        """Test that the documentation version template tag returns correctly"""
        self.assertEqual(arius_extras.arius_docs_version(), version.ariusDocsVersion())

    def test_hash(self):
        """Test that the commit hash template tag returns correctly"""
        result_hash = arius_extras.arius_commit_hash()
    ...
    def test_date(self):
        ...
        d = arius_extras.arius_commit_date()
        if settings.DOCKER:  # pragma: no cover
            # Testing inside docker environment *may* return an empty git commit hash
            # In such a case, skip this check
            pass
    ...
    def test_github(self):
        ...
        self.assertIn('github.com', arius_extras.arius_github_url())

    def test_docs(self):
        """Test that the documentation URL template tag returns correctly"""
        self.assertIn('docs.arius.org', arius_extras.arius_docs_url())

    def test_keyvalue(self):
        """Test keyvalue template tag"""
        self.assertEqual(arius_extras.keyvalue({'a': 'a'}, 'a'), 'a')

    def test_mail_configured(self):
        """Test that mail configuration returns False"""
        self.assertEqual(arius_extras.mail_configured(), False)

    def test_user_settings(self):
        """Test user settings"""
        result = arius_extras.user_settings(self.user)
        self.assertEqual(len(result), len(AriusUserSetting.SETTINGS))

    def test_global_settings(self):
        """Test global settings"""
        result = arius_extras.global_settings()
        self.assertEqual(len(result), len(AriusSetting.SETTINGS))

    def test_visible_global_settings(self):
        """Test that hidden global settings are actually hidden"""
        result = arius_extras.visible_global_settings()
        ...
        for val in AriusSetting.SETTINGS.values():
...
class PartSettingsTest(AriusTestCase):
    ...
    def test_custom(self):
        ...
        for val in [True, False]:
            AriusSetting.set_setting('PART_COMPONENT', val, self.user)
            AriusSetting.set_setting('PART_PURCHASEABLE', val, self.user)
            AriusSetting.set_setting('PART_SALABLE', val, self.user)
            AriusSetting.set_setting('PART_TRACKABLE', val, self.user)
            AriusSetting.set_setting('PART_ASSEMBLY', val, self.user)
            AriusSetting.set_setting('PART_TEMPLATE', val, self.user)

            self.assertEqual(val, AriusSetting.get_setting('PART_COMPONENT'))
            self.assertEqual(val, AriusSetting.get_setting('PART_PURCHASEABLE'))
            self.assertEqual(val, AriusSetting.get_setting('PART_SALABLE'))
            self.assertEqual(val, AriusSetting.get_setting('PART_TRACKABLE'))
    ...
    def test_duplicate_ipn(self):
        ...
        AriusSetting.set_setting('PART_ALLOW_DUPLICATE_IPN', False, self.user)
...
class PartSubscriptionTests(AriusTestCase):
...
class BaseNotificationIntegrationTest(AriusTestCase):
