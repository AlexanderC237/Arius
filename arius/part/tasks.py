...
import part.models
import stock.models
from arius.tasks import ScheduledTask, scheduled_task
...
def notify_low_stock(part: part.models.Part):
    ...
    context = {
        ...
        'name': name,
        'message': message,
        'link': arius.helpers_model.construct_absolute_url(part.get_absolute_url()),
        'template': {
...
def check_missing_pricing(limit=250):
    ...
    days = int(common.models.AriusSetting.get_setting('PRICING_UPDATE_DAYS', 30))
...
def generate_stocktake_report(**kwargs):
    ...
    for p in parts:
        ...
        dataset.append([
            ...
            stocktake.quantity,
            arius.helpers.normalize(stocktake.cost_min.amount),
            arius.helpers.normalize(stocktake.cost_max.amount),
        ])
    ...
    filename = f"Arius_Stocktake_{today}.csv"
...
def scheduled_stocktake_reports():
    ...
    delete_n_days = int(common.models.AriusSetting.get_setting('STOCKTAKE_DELETE_REPORT_DAYS', 30, cache=False))
    ...
    if not common.models.AriusSetting.get_setting('STOCKTAKE_ENABLE', False, cache=False):
        logger.info("Stocktake functionality is not enabled - exiting")
        return

    report_n_days = int(common.models.AriusSetting.get_setting('STOCKTAKE_AUTO_DAYS', 0, cache=False))
    ...
    last_report = common.models.AriusSetting.get_setting('STOCKTAKE_RECENT_REPORT', '', cache=False)
    ...
    common.models.AriusSetting.set_setting('STOCKTAKE_RECENT_REPORT', datetime.now().isoformat(), None)
