...
from arius.status_codes import (BuildStatus, PurchaseOrderStatusGroups,
                                StockStatus)
from arius.unit_test import AriusAPITestCase
...
class PartCategoryAPITest(AriusAPITestCase):
...
class PartOptionsAPITest(AriusAPITestCase):
...
class PartAPITestBase(AriusAPITestCase):
...
class PartNotesTests(AriusAPITestCase):
...
class PartPricingDetailTests(AriusAPITestCase):
...
class PartAPIAggregationTest(AriusAPITestCase):
    ...
    def test_allocation_annotations(self):
        ...
        Ref: https://github.com/ariustechnology/Arius/pull/2797
        """
...
class BomItemTest(AriusAPITestCase):
...
class PartAttachmentTest(AriusAPITestCase):
...
class PartInternalPriceBreakTest(AriusAPITestCase):
...
class PartStocktakeTest(AriusAPITestCase):
    ...
    def test_report_generate(self):
        ...
        AriusSetting.set_setting('STOCKTAKE_ENABLE', False, None)
        self.assignRole('stocktake.add')
        response = self.post(url, data={}, expected_code=400)

        self.assertIn('Stocktake functionality is not enabled', str(response.data))

        AriusSetting.set_setting('STOCKTAKE_ENABLE', True, None)
        response = self.post(url, data={}, expected_code=400)
        self.assertIn('Background worker check failed', str(response.data))


class PartMetadataAPITest(AriusAPITestCase):
