...
from arius import helpers, validators
from arius.fields import AriusURLField
from arius.helpers import (decimal2money, decimal2string, normalize,
                           str2bool)
from arius.models import (DataImportMixin, AriusAttachment,
                          AriusBarcodeMixin, AriusNotesMixin,
                          AriusTree, MetadataMixin)
from arius.status_codes import (BuildStatusGroups, PurchaseOrderStatus,
...
class PartCategory(MetadataMixin, AriusTree):
...
@cleanup.ignore
class Part(AriusBarcodeMixin, AriusNotesMixin, MetadataMixin, MPTTModel):
    ...
    def validate_ipn(self, raise_error=True):
        ...
        pattern = common.models.AriusSetting.get_setting('PART_IPN_REGEX', '', create=False).strip()
    ...
    def validate_serial_number(self, serial: str, stock_item=None, check_duplicates=True, raise_error=False, **kwargs):
        ...
        if common.models.AriusSetting.get_setting('SERIAL_NUMBER_GLOBALLY_UNIQUE', False):
    ...
    @property
    def full_name(self):
        """Format a 'full name' for this Part based on the format PART_NAME_FORMAT defined in arius settings.
        ...
        """
        full_name_pattern = AriusSetting.get_setting('PART_NAME_FORMAT')
    ...
    def validate_unique(self, exclude=None):
        ...
        allow_duplicate_ipn = common.models.AriusSetting.get_setting('PART_ALLOW_DUPLICATE_IPN')
    ...
    def schedule_pricing_update(self, create: bool = False, test: bool = False):
        ...
        Ref: https://github.com/ariustechnology/Arius/pull/3986
...
def after_save_part(sender, instance: Part, created, **kwargs):
    ...
    if instance and not created and not arius.ready.isImportingData():
        # Check part stock only if we are *updating* the part (not creating it)

        # Run this check in the background
        try:
            arius.tasks.offload_task(part_tasks.notify_low_stock_if_required, instance)
...
class PartPricing(common.models.MetaMixin):
    ...
    def schedule_for_update(self, counter: int = 0, test: bool = False):
        ...
        if arius.ready.isRunningMigrations():
            return
    ...
    def update_purchase_cost(self, save=True):
        ...
        if AriusSetting.get_setting('PRICING_USE_STOCK_PRICING', True, cache=False):

            items = self.part.stock_items.all()

            # Limit to stock items updated within a certain window
            days = int(AriusSetting.get_setting('PRICING_STOCK_ITEM_AGE_DAYS', 0, cache=False))
    ...
    def update_internal_cost(self, save=True):
        ...
        if AriusSetting.get_setting('PART_INTERNAL_PRICE', False, cache=False):
    ...
    def update_variant_cost(self, save=True):
        ...
        active_only = AriusSetting.get_setting('PRICING_ACTIVE_VARIANTS', False)
    ...
    def update_overall_cost(self):
        ...
        purchase_history_override = AriusSetting.get_setting('PRICING_PURCHASE_HISTORY_OVERRIDES_SUPPLIER', False, cache=False)

        if AriusSetting.get_setting('PRICING_USE_SUPPLIER_PRICING', True, cache=False):
        ...
        if AriusSetting.get_setting('PRICING_USE_VARIANT_PRICING', True, cache=False):
        ...
        if AriusSetting.get_setting('PART_BOM_USE_INTERNAL_PRICE', False, cache=False):
...
class PartStocktake(models.Model):
    ...
    cost_min = arius.fields.AriusModelMoneyField(
        null=True, blank=True,
        verbose_name=_('Minimum Stock Cost'),
        help_text=_('Estimated minimum cost of stock on hand'),
    )

    cost_max = arius.fields.AriusModelMoneyField(
        null=True, blank=True,
...
class PartAttachment(AriusAttachment):
...
def post_save_part_parameter_template(sender, instance, created, **kwargs):
    ...
    if arius.ready.canAppAccessDatabase() and not arius.ready.isImportingData():
...
class PartParameter(MetadataMixin, models.Model):
    ...
    def clean(self):
        ...
        if self.template.units:
            try:
                arius.conversion.convert_physical_value(self.data, self.template.units)
            except ValidationError as e:
                raise ValidationError({
                    'data': e.message
                })
    ...
    def calculate_numeric_value(self):
        ...
        if self.template.units:
            try:
                converted = arius.conversion.convert_physical_value(self.data, self.template.units)
...
class BomItem(DataImportMixin, MetadataMixin, models.Model):
    ...
    def price_range(self, internal=False):
        ...
        use_internal = common.models.AriusSetting.get_setting('PART_BOM_USE_INTERNAL_PRICE', False, cache=False)
