...
import arius.helpers_model
from common.models import ColorTheme, AriusSetting, AriusUserSetting
from common.settings import currency_code_default
from arius import settings, version
from plugin import registry
from plugin.models import NotificationUserSetting, PluginSetting
from plugin.plugin import AriusPlugin
...
def decimal(x, *args, **kwargs):
    ...
    return arius.helpers.decimal2string(x)
...
def render_date(context, date_object):
    ...
    if user_date_format is None:
        ...
        if user and user.is_authenticated:
            ...
            user_date_format = AriusUserSetting.get_setting('DATE_DISPLAY_FORMAT', user=user)
        else:
...
def render_currency(money, **kwargs):
    ...
    return arius.helpers_model.render_currency(money, **kwargs)


@register.simple_tag()
def str2bool(x, *args, **kwargs):
    """Convert a string to a boolean value."""
    return arius.helpers.str2bool(x)
...
def part_allocation_count(build, part, *args, **kwargs):
    ...
    return arius.helpers.decimal2string(build.getAllocatedQuantity(part))


@register.simple_tag()
def arius_in_debug_mode(*args, **kwargs):
    """Return True if the server is running in DEBUG mode."""
    return djangosettings.DEBUG


@register.simple_tag()
def arius_show_about(user, *args, **kwargs):
    """Return True if the about modal should be shown."""
    if AriusSetting.get_setting('ARIUS_RESTRICT_ABOUT'):
...
@register.simple_tag()
def arius_docker_mode(*args, **kwargs):
...
@register.simple_tag()
def arius_db_engine(*args, **kwargs):
    """Return the arius database backend e.g. 'postgresql'."""
...
@register.simple_tag()
def arius_instance_name(*args, **kwargs):
    """Return the InstanceName associated with the current database."""
    return version.ariusInstanceName()


@register.simple_tag()
def arius_title(*args, **kwargs):
    """Return the title for the current instance - respecting the settings"""
    return version.ariusInstanceTitle()


@register.simple_tag()
def arius_logo(**kwargs):
    """Return the arius logo, *or* a custom logo if the user has provided one.

    Returns a path to an image file, which can be rendered in the web interface
    """

    return arius.helpers.getLogoImage(**kwargs)


@register.simple_tag()
def arius_splash(**kwargs):
    """Return the URL for the arius splash screen, *or* a custom screen if the user has provided one."""

    return arius.helpers.getSplashScren(**kwargs)


@register.simple_tag()
def arius_base_url(*args, **kwargs):
    """Return the base URL of the arius server"""
    return arius.helpers_model.get_base_url()
...
@register.simple_tag()
def arius_version(shortstring=False, *args, **kwargs):
    """Return arius version string."""
    if shortstring:
        return _("{title} v{version}".format(
            title=version.ariusInstanceTitle(),
            version=version.ariusVersion()
        ))
    return version.ariusVersion()


@register.simple_tag()
def arius_is_development(*args, **kwargs):
    """Returns True if this is a development version of arius"""
    return version.isAriusDevelopmentVersion()


@register.simple_tag()
def arius_is_release(*args, **kwargs):
    """Returns True if this is a release version of arius"""
    return not version.isAriusDevelopmentVersion()


@register.simple_tag()
def arius_docs_version(*args, **kwargs):
    """Returns the arius documentation version"""
    return version.ariusDocsVersion()


@register.simple_tag()
def arius_api_version(*args, **kwargs):
    """Return arius API version."""
    return version.ariusApiVersion()


@register.simple_tag()
def django_version(*args, **kwargs):
    """Return Django version string."""
    return version.ariusDjangoVersion()


@register.simple_tag()
def arius_commit_hash(*args, **kwargs):
    """Return arius git commit hash string."""
    return version.ariusCommitHash()


@register.simple_tag()
def arius_commit_date(*args, **kwargs):
    """Return arius git commit date string."""
    return version.ariusCommitDate()


@register.simple_tag()
def arius_installer(*args, **kwargs):
    """Return arius package installer string."""
    return version.ariusInstaller()


@register.simple_tag()
def arius_branch(*args, **kwargs):
    """Return arius git branch string."""
    return version.ariusBranch()


@register.simple_tag()
def arius_target(*args, **kwargs):
    """Return arius target string."""
    return version.ariusTarget()


@register.simple_tag()
def arius_platform(*args, **kwargs):
    """Return arius platform string."""
    return version.ariusPlatform()


@register.simple_tag()
def arius_github_url(*args, **kwargs):
    """Return URL for arius github site."""
    return "https://github.com/ariustechnology/Arius/"


@register.simple_tag()
def arius_docs_url(*args, **kwargs):
    """Return URL for arius documentation site."""
    tag = version.ariusDocsVersion()

    return f"https://docs.arius.org/en/{tag}"


@register.simple_tag()
def arius_credits_url(*args, **kwargs):
    """Return URL for arius credits site."""
    return "https://docs.arius.org/en/latest/credits/"
...
def setting_object(key, *args, **kwargs):
    ...
    if 'plugin' in kwargs:
        # Note, 'plugin' is an instance of an AriusPlugin class

        plugin = kwargs['plugin']
        if issubclass(plugin.__class__, AriusPlugin):
        ...
        return AriusUserSetting.get_setting_object(key, user=kwargs['user'], cache=cache)

    else:
        return AriusSetting.get_setting_object(key, cache=cache)
...
def settings_value(key, *args, **kwargs):
    ...
    if 'user' in kwargs:
        if not kwargs['user'] or (kwargs['user'] and kwargs['user'].is_authenticated is False):
            return AriusUserSetting.get_setting(key)
        return AriusUserSetting.get_setting(key, user=kwargs['user'])

    return AriusSetting.get_setting(key)


@register.simple_tag()
def user_settings(user, *args, **kwargs):
    """Return all USER settings as a key:value dict."""
    return AriusUserSetting.allValues(user=user)


@register.simple_tag()
def global_settings(*args, **kwargs):
    """Return all GLOBAL arius settings as a key:value dict."""
    return AriusSetting.allValues()


@register.simple_tag()
def visible_global_settings(*args, **kwargs):
    """Return any global settings which are not marked as 'hidden'."""
    return AriusSetting.allValues(exclude_hidden=True)
...
def progress_bar(val, max_val, *args, **kwargs):
    ...
    val = arius.helpers.normalize(val)
    max_val = arius.helpers.normalize(max_val)
...
def get_color_theme_css(username):
    ...
    arius_css_sheet = os.path.join('css', 'color-themes', user_theme_name + '.css')

    # Build static URL
    arius_css_static_url = os.path.join(settings.STATIC_URL, arius_css_sheet)

    return arius_css_static_url
...
@register.simple_tag()
def arius_customize(reference, *args, **kwargs):
