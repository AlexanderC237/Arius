...
from arius.status_codes import BuildStatus, StockStatus, StockHistoryCode, BuildStatusGroups
...
class Build(MPTTModel, arius.models.AriusBarcodeMixin, arius.models.AriusNotesMixin, arius.models.MetadataMixin, arius.models.ReferenceIndexingMixin):
    ...
    def getNextBuildNumber(cls):
        ...
        while 1:
            new_ref = arius.helpers.increment(new_ref)
    ...
    def complete_build(self, user):
        ...
        context = {
            ...
            'link': arius.helpers_model.construct_absolute_url(self.get_absolute_url()),
            'template': {
...
def after_save_build(sender, instance: Build, created: bool, **kwargs):
    ...
    if arius.ready.isImportingData() or not arius.ready.canAppAccessDatabase(allow_test=True):
        return
    ...
    if instance:
        ...
        if created:
            ...
            arius.tasks.offload_task(build_tasks.check_build_stock, instance)

            # Notify the responsible users that the build order has been created
            arius.helpers_model.notify_responsible(instance, sender, exclude=instance.issued_by)
...
class BuildOrderAttachment(arius.models.AriusAttachment):
...
class BuildItem(arius.models.MetadataMixin, models.Model):
    ...
    def clean(self):
        ...
        try:
            ...
            if self.quantity > self.stock_item.quantity:
                ...
                q = arius.helpers.normalize(self.quantity)
                a = arius.helpers.normalize(self.stock_item.quantity)
    ...
    def getStockItemThumbnail(self):
        ...
        if thumb_url is not None:
            return arius.helpers.getMediaUrl(thumb_url)
        else:
