...
function createNewModal(options={}) {
    ...
    var html = `
    <div class='modal fade modal-fixed-footer modal-primary arius-modal' role='dialog' id='modal-form-${id}' tabindex='-1'>
...
function reloadFieldOptions(fieldName, options) {
    ...
    ariusGet(options.url, options.params, {
    ...
    var ajax_data = {
        ...
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    '{% trans "No Response" %}',
                    '{% trans "No response from the arius server" %}',
                );
