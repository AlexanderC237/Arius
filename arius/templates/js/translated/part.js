...
function partStockLabel(part, options={}) {
    ...
    if (options.showDemandInfo) {
        ...
        ariusGet(`{% url "api-part-list" %}${part.pk}/requirements/`, {}, {
            async: false,
            success: function(response) {
        ...
        template: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                if (value) {
                    ...
                    ariusGet(`{% url "api-part-parameter-template-list" %}${value}/`, {}, {
                        async: false,
                        success: function(response) {
...
function loadParametricPartTable(table, options={}) {
    ...
    $(table).ariusTable({
        ...
        // Ref: https://github.com/ariustechnology/Arius/issues/4851
        // filterControl: true,
...
function loadPartTable(table, url, options={}) {
    ...
    var grid_view = options.gridView && ariusLoad('part-grid-view') == 1;
    ...
    $(table).ariusTable({
        ...
        onPostBody: function() {
            grid_view = ariusLoad('part-grid-view') == 1;
            if (grid_view) {
...
function loadPartCategoryTable(table, options) {
    ...
    var tree_view = options.allowTreeView && ariusLoad('category-tree-view') == 1;

    if (tree_view) {
        params.cascade = true;
        params.depth = global_settings.ARIUS_TREE_DEPTH;
    }
    ...
    table.ariusTable({
        ...
        buttons: options.allowTreeView ? [
            {
                ...
                event: () => {
                    ariusSave('category-tree-view', 0);
            ...
            },
            {
                ...
                event: () => {
                    ariusSave('category-tree-view', 1);
        ...
        onPostBody: function() {
            ...
            if (options.allowTreeView) {
                ...
                tree_view = ariusLoad('category-tree-view') == 1;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row._level >= global_settings.ARIUS_TREE_DEPTH && !row.subReceived) {
                        if (row.subRequested) {
...
function loadPartSchedulingChart(canvas_id, part_id) {
    ...
    ariusGet(`{% url "api-part-list" %}${part_id}/`, {}, {
        async: false,
        success: function(response) {
            part_info = response;
        }
