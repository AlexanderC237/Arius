...
/*
 * Perform a GET request to the arius server
 */
function ariusGet(url, filters={}, options={}) {

    if (!url) {
        console.error('ariusGet called without url');
        return;
    }
 ...
 * contentType: false
 */
function ariusFormDataUpload(url, data, options={}) {

    if (!url) {
        console.error('ariusFormDataUpload called without url');
        return;
    }
...
/*
 * Perform a PUT or PATCH request to the arius server
 */
function ariusPut(url, data={}, options={}) {

    if (!url) {
        console.error('ariusPut called without url');
        return;
    }
 ...
 */
function ariusDelete(url, options={}) {

    if (!url) {
        console.error('ariusDelete called without url');
        return;
    }
...
function showApiError(xhr, url) {
    ...
    case 0:
        ...
        message = '{% trans "No response from the arius server" %}';
        break;
    // Bad request
    case 400:
