...
function constructFormBody(fields, options) {
    ...
    if (options.confirm && global_settings.ARIUS_REQUIRE_CONFIRM) {
...
function submitFormData(fields, options) {
    ...
    var upload_func = ariusPut;

    if (has_files) {
        upload_func = ariusFormDataUpload;
        data = form_data;
    }
...
function initializeRelatedField(field, fields, options={}) {
    ...
    select.select2({
        ...
        ajax: {
            ...
            processResults: function(response) {
                // Convert the returned arius data into select2-friendly format
    ...
    if (field.value) {
        ...
        ariusGet(field.api_url, filters || {}, {
            success: function(data) {
 ...
 * - model: The name of the arius model e.g. 'stockitem'
