...
function setupNotesField(element, url, options={}) {
    ...
    const mde = new EasyMDE({
        ...
        imageUploadFunction: function(imageFile, onSuccess, onError) {
            // Attempt to upload the image to the arius server
            var form_data = new FormData();

            form_data.append('image', imageFile);

            ariusFormDataUpload('{% url "api-notes-image-list" %}', form_data, {
                success: function(response) {
 ...
 * attachClipboard('.clip-btn') in script/arius/arius.js is altered
 */
