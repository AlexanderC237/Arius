...
function constructOrderTableButtons(options={}) {
    ...
    if (!display_mode) {
        display_mode = ariusLoad(key, 'list');
    }
 ...
 * - The table has been loaded using the ariusTable() function, not bootstrapTable()
...
/*
 * Convert bootstrap-table style parameters to "arius" style
*/
function convertQueryParameters(params, filters) {
    ...
    // Add "order" back in (if it was originally specified by arius)
    // Annoyingly, "order" shadows some field names in arius...
    if ('order' in filters) {
 ...
 */
$.fn.ariusTable = function(options) {
    ...
    if (!options.disablePagination) {
        ...
        options.pageSize = options.pageSize || ariusLoad(varName, 25);
    ...
    options.onColumnSwitch = function() {
        ...
        ariusSave(`table_columns_${tableName}`, text);
    };
    ...
    var visibleColumns = ariusLoad(`table_columns_${tableName}`, null);
