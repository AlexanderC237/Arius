...
from arius.helpers import (extract_serial_numbers, hash_barcode, normalize,
                           str2bool)
from arius.serializers import (AriusAttachmentSerializer,
                               AriusCurrencySerializer,
                               AriusDecimalField,
                               AriusModelSerializer,
                               AriusMoneySerializer)
from arius.status_codes import (PurchaseOrderStatusGroups,
...
class TotalPriceMixin(serializers.Serializer):
    ...
    total_price = AriusMoneySerializer(
        allow_null=True,
        read_only=True,
    )

    order_currency = AriusCurrencySerializer(
        allow_blank=True,
        allow_null=True,
        required=False,
...
class AbstractExtraLineSerializer(serializers.Serializer):
    ...
    price = AriusMoneySerializer(
        allow_null=True
    )

    price_currency = AriusCurrencySerializer()
...
class PurchaseOrderSerializer(TotalPriceMixin, AbstractOrderSerializer, AriusModelSerializer):
...
class PurchaseOrderLineItemSerializer(AriusModelSerializer):
    ...
    purchase_price = AriusMoneySerializer(allow_null=True)

    destination_detail = stock.serializers.LocationBriefSerializer(source='get_destination', read_only=True)

    purchase_price_currency = AriusCurrencySerializer(help_text=_('Purchase price currency'))
...
class PurchaseOrderExtraLineSerializer(AbstractExtraLineSerializer, AriusModelSerializer):
...
class PurchaseOrderAttachmentSerializer(AriusAttachmentSerializer):
    ...
    class Meta:
        ...
        fields = AriusAttachmentSerializer.attachment_fields([
            'order',
        ])


class SalesOrderSerializer(TotalPriceMixin, AbstractOrderSerializer, AriusModelSerializer):
...
class SalesOrderAllocationSerializer(AriusModelSerializer):
...
class SalesOrderLineItemSerializer(AriusModelSerializer):
    ...
    quantity = AriusDecimalField()

    allocated = serializers.FloatField(source='allocated_quantity', read_only=True)

    shipped = AriusDecimalField(read_only=True)

    sale_price = AriusMoneySerializer(allow_null=True)

    sale_price_currency = AriusCurrencySerializer(help_text=_('Sale price currency'))


class SalesOrderShipmentSerializer(AriusModelSerializer):
...
class SalesOrderExtraLineSerializer(AbstractExtraLineSerializer, AriusModelSerializer):
...
class SalesOrderAttachmentSerializer(AriusAttachmentSerializer):
...
class ReturnOrderSerializer(AbstractOrderSerializer, TotalPriceMixin, AriusModelSerializer):
...
class ReturnOrderLineItemSerializer(AriusModelSerializer):
    ...
    price = AriusMoneySerializer(allow_null=True)
    price_currency = AriusCurrencySerializer(help_text=_('Line price currency'))


class ReturnOrderExtraLineSerializer(AbstractExtraLineSerializer, AriusModelSerializer):
...
class ReturnOrderAttachmentSerializer(AriusAttachmentSerializer):
