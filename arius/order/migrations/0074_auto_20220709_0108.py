...
def update_salesorder_reference(apps, schema_editor):
    ...
    AriusSetting = apps.get_model('common', 'ariussetting')

    try:
        prefix = AriusSetting.objects.get(key='SALESORDER_REFERENCE_PREFIX').value
    except Exception:
        prefix = 'SO-'
    ...
    try:
        setting = AriusSetting.objects.get(key='SALESORDER_REFERENCE_PATTERN')
        setting.value = pattern
        setting.save()
    except AriusSetting.DoesNotExist:
        setting = AriusSetting.objects.create(
...
def update_purchaseorder_reference(apps, schema_editor):
    ...
    try:
        prefix = AriusSetting.objects.get(key='PURCHASEORDER_REFERENCE_PREFIX').value
    except Exception:
        prefix = 'PO-'
    ...
    # Create or update the BuildOrder.reference pattern
    try:
        setting = AriusSetting.objects.get(key='PURCHASEORDER_REFERENCE_PATTERN')
        setting.value = pattern
        setting.save()
    except AriusSetting.DoesNotExist:
        setting = AriusSetting.objects.create(
