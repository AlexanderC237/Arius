...
from arius.exceptions import log_error
from arius.fields import (AriusModelMoneyField, AriusURLField,
                          RoundingDecimalField)
from arius.helpers import decimal2string
from arius.helpers_model import getSetting, notify_responsible
from arius.models import (AriusAttachment, AriusBarcodeMixin,
                          AriusNotesMixin, MetadataMixin,
                          ReferenceIndexingMixin)
from arius.status_codes import (PurchaseOrderStatus,
...
class TotalPriceMixin(models.Model):
    ...
    total_price = AriusModelMoneyField(
    ...
    order_currency = models.CharField(
        ...
        validators=[arius.validators.validate_currency_code]
    )
...
class Order(AriusBarcodeMixin, AriusNotesMixin, MetadataMixin, ReferenceIndexingMixin):
    ...
    link = AriusURLField(blank=True, verbose_name=_('Link'), help_text=_('Link to external page'))
...
class PurchaseOrder(TotalPriceMixin, Order):
    ...
    def place_order(self):
        ...
        if self.status == PurchaseOrderStatus.PENDING:
            ...
            notify_responsible(
                ...
                content=AriusNotificationBodies.NewOrder
            )
    ...
    def receive_line_item(self, line, location, quantity, user, status=StockStatus.OK.value, **kwargs):
        ...
        try:
            ...
            quantity = arius.helpers.clean_decimal(quantity)
        except TypeError:
            raise ValidationError({
        ...
        notify_responsible(
            ...
            PurchaseOrder,
            exclude=user,
            content=AriusNotificationBodies.ItemsReceived,
        )
...
def after_save_sales_order(sender, instance: SalesOrder, created: bool, **kwargs):
    ...
    """
    if not arius.ready.canAppAccessDatabase(allow_test=True) or arius.ready.isImportingData():
        return
...
class PurchaseOrderAttachment(AriusAttachment):
...
class SalesOrderAttachment(AriusAttachment):
...
class SalesOrderLineItem(OrderLineItem):
    ...
    sale_price = AriusModelMoneyField(
        max_digits=19,
        decimal_places=6,
        null=True, blank=True,
...
class SalesOrderShipment(AriusNotesMixin, MetadataMixin, models.Model):
...
class ReturnOrder(TotalPriceMixin, Order):
    ...
    def receive_line_item(self, line, location, user, note=''):
        ...
        notify_responsible(
            ...
            ReturnOrder,
            exclude=user,
            content=AriusNotificationBodies.ReturnOrderItemsReceived,
        )
...
class ReturnOrderAttachment(AriusAttachment):
