...
import arius.status_codes
import part.models as part_models
import stock.filters
from company.serializers import SupplierPartSerializer
from arius.models import extract_int
from arius.serializers import (AriusCurrencySerializer,
                               AriusDecimalField)
...
class LocationBriefSerializer(arius.serializers.AriusModelSerializer):
...
class StockItemTestResultSerializer(arius.serializers.AriusModelSerializer):
    ...
    user_detail = arius.serializers.UserSerializer(source='user', read_only=True)

    key = serializers.CharField(read_only=True)

    attachment = arius.serializers.AriusAttachmentSerializerField(required=False)


class StockItemSerializerBrief(arius.serializers.AriusModelSerializer):
...
class StockItemSerializer(arius.serializers.AriusTagModelSerializer):
    ...
    def annotate_queryset(queryset):
        ...
        stale_days = common.models.AriusSetting.get_setting('STOCK_STALE_DAYS')
    ...
    purchase_price = arius.serializers.AriusMoneySerializer(
        label=_('Purchase Price'),
        allow_null=True,
        help_text=_('Purchase price of this stock item, per unit or pack'),
    )

    purchase_price_currency = AriusCurrencySerializer(help_text=_('Purchase currency of this stock item'))
...
class SerializeStockItemSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            serials = arius.helpers.extract_serial_numbers(
                serial_numbers,
                quantity,
...
class StockChangeStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=arius.status_codes.StockStatus.items(),
        default=arius.status_codes.StockStatus.OK.value,
        label=_('Status'),
    )
    ...
    def save(self):
        ...
        for item in items:
            ...
            transaction_notes.append(
                StockItemTracking(
                    item=item,
                    tracking_type=arius.status_codes.StockHistoryCode.EDITED.value,
                    date=now,
                    deltas=deltas,
                    user=user,
                    notes=note,
...
class LocationTreeSerializer(arius.serializers.AriusModelSerializer):
...
class LocationSerializer(arius.serializers.AriusTagModelSerializer):
...
class StockItemAttachmentSerializer(arius.serializers.AriusAttachmentSerializer):
...
class StockTrackingSerializer(arius.serializers.AriusModelSerializer):
    ...
    user_detail = arius.serializers.UserSerializer(source='user', many=False, read_only=True)
