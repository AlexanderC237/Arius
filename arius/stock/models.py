...
from arius.fields import AriusModelMoneyField, AriusURLField
from arius.models import (AriusAttachment, AriusBarcodeMixin,
                          AriusNotesMixin, AriusTree,
                          MetadataMixin, extract_int)
from arius.status_codes import (SalesOrderStatusGroups, StockHistoryCode,
...
class StockLocation(AriusBarcodeMixin, MetadataMixin, AriusTree):
    ...
    def check_ownership(self, user):
        ...
        ownership_enabled = common.models.AriusSetting.get_setting('STOCK_OWNERSHIP_CONTROL')
...
def generate_batch_code():
    ...
    batch_template = common.models.AriusSetting.get_setting('STOCK_BATCH_CODE_TEMPLATE', '')
...
def default_delete_on_deplete():
    ...
    try:
        return common.models.AriusSetting.get_setting('STOCK_DELETE_DEPLETED_DEFAULT', True)
    except (IntegrityError, OperationalError):
        # Revert to original default behaviour
        return True


class StockItem(AriusBarcodeMixin, AriusNotesMixin, MetadataMixin, common.models.MetaMixin, MPTTModel):
    ...
    Attributes:
        ...
        status: Status of this StockItem (ref: arius.status_codes.StockStatus)
    ...
    purchase_price = AriusModelMoneyField(
        max_digits=19,
        decimal_places=6,
        blank=True,
        null=True,
    ...
    def __str__(self):
        ...
        if self.part.trackable and self.serial:
            ...
            s = '{n} x {part}'.format(
                n=arius.helpers.decimal2string(self.quantity),
                part=self.part.full_name)
    ...
    def testResultMap(self, **kwargs):
        ...
        for result in results:
            key = arius.helpers.generateTestKey(result.test)
    ...
    def requiredTestStatus(self):
        ...
        for test in required:
            key = arius.helpers.generateTestKey(test.test_name)
    ...
    def available_test_reports(self):
        ...
        for test_report in report.models.TestReport.objects.filter(enabled=True):
            ...
            try:
                filters = arius.helpers.validateFilterString(test_report.filters)
    ...
    def available_labels(self):
        ...
        for lbl in label.models.StockItemLabel.objects.filter(enabled=True):
            ...
            try:
                filters = arius.helpers.validateFilterString(lbl.filters)
...
def after_delete_stock_item(sender, instance: StockItem, **kwargs):
    ...
    if not arius.ready.isImportingData():
        # Run this check in the background
        arius.tasks.offload_task(part_tasks.notify_low_stock_if_required, instance.part)

        # Schedule an update on parent part pricing
        if arius.ready.canAppAccessDatabase(allow_test=True):
...
class StockItemAttachment(AriusAttachment):
...
class StockItemTestResult(MetadataMixin, models.Model):
    ...
    def key(self):
        """Return key for test."""
        return arius.helpers.generateTestKey(self.test)
