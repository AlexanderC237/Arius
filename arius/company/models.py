...
from arius.fields import AriusURLField, RoundingDecimalField
...
class Company(AriusNotesMixin, MetadataMixin, models.Model):
    ...
    link = AriusURLField(blank=True, verbose_name=_('Link'), help_text=_('Link to external company information'))
    ...
    def get_image_url(self):
        ...
        if self.image:
            return arius.helpers.getMediaUrl(self.image.url)
        else:
            return arius.helpers.getBlankImage()

    def get_thumbnail_url(self):
        """Return the URL for the thumbnail image for this Company."""
        if self.image:
            return arius.helpers.getMediaUrl(self.image.thumbnail.url)
        else:
            return arius.helpers.getBlankThumbnail()
...
class CompanyAttachment(AriusAttachment):
...
class ManufacturerPartAttachment(AriusAttachment):
...
class SupplierPart(MetadataMixin, AriusBarcodeMixin, common.models.MetaMixin):
    ...
    def clean(self):
        ...
        if self.pack_quantity and self.part:
            try:
                ...
                native_value = arius.conversion.convert_physical_value(
