...
def asset(filename):
    ...
    debug_mode = AriusSetting.get_setting('REPORT_DEBUG_MODE')
...
def uploaded_image(filename, replace_missing=True, replacement_file='blank_image.png', validate=True):
    ...
    if exists and validate and not arius.helpers.TestIfImage(full_path):
...
def logo_image(**kwargs):
    ...
    - Otherwise, return a path to the default arius logo
    """

    # If in debug mode, return URL to the image, not a local file
    debug_mode = AriusSetting.get_setting('REPORT_DEBUG_MODE')

    return arius.helpers.getLogoImage(as_file=not debug_mode, **kwargs)


@register.simple_tag()
def internal_link(link, text):
    """Make a <a></a> href which points to an arius URL.

    Uses the arius.helpers_model.construct_absolute_url function to build the URL.
    """
    text = str(text)

    url = arius.helpers_model.construct_absolute_url(link)
