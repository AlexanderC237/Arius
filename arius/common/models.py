...
class MetaMixin(models.Model):
    """A base class for arius models to include shared meta fields.
...
class ProjectCode(arius.models.MetadataMixin, models.Model):
...
class BaseAriusSetting(models.Model):
    """An base AriusSetting object is a key:value pair used for storing single values (e.g. one-off settings values).
    ...
    class Meta:
        """Meta options for BaseAriusSetting -> abstract stops creation of database entry."""
    ...
    def allValues(cls, exclude_hidden=False, **kwargs):
        ...
        for key, value in settings.items():
            ...
            if cls.is_protected(key):
                ...
                value = arius.helpers.str2bool(value)
    ...
    def get_setting_object(cls, key, **kwargs):
        """Return an AriusSetting object matching the given key.
        ...
        if not setting:
            ...
            if arius.ready.isImportingData() or not arius.ready.canAppAccessDatabase(allow_test=True, allow_shell=True):
                create = False
    ...
    def run_validator(self, validator):
        """Run a validator against the 'value' field for this AriusSetting object."""
        ...
        if validator is bool:
            ...
            if arius.helpers.is_bool(value):
    ...
    def as_bool(self):
        ...
        """
        return arius.helpers.str2bool(self.value)
...
def update_exchange_rates(setting):
    ...
    if not arius.ready.canAppAccessDatabase():
        return

    arius.tasks.update_exchange_rates()


class AriusSetting(BaseAriusSetting):
    """An AriusSetting object is a key:value pair used for storing single values (e.g. one-off settings values).
    ...
    class Meta:
        """Meta options for AriusSetting."""

        verbose_name = "arius Setting"
        verbose_name_plural = "arius Settings"
    ...
    def save(self, *args, **kwargs):
        ...
        if self.requires_restart() and not arius.ready.isImportingData():
            AriusSetting.set_setting('SERVER_RESTART_REQUIRED', True, None)
    ...
    SETTINGS = {
        ...
        'ARIUS_DOWNLOAD_FROM_URL_USER_AGENT': {
        ...
        'ARIUS_DELETE_NOTIFICATIONS_DAYS': {
        ...
        'PART_NAME_FORMAT': {
            ...
            'validator': arius.validators.validate_part_name_format
        },
        ...
        'PLUGIN_ON_STARTUP': {
            ...
            'default': str(os.getenv('ARIUS_DOCKER', False)).lower() in ['1', 'true'],
            'validator': bool,
    ...
    def requires_restart(self):
        ...
        options = AriusSetting.SETTINGS.get(self.key, None)
...
class AriusUserSetting(BaseAriusSetting):
    """An AriusSetting object with a usercontext."""

    class Meta:
        """Meta options for AriusUserSetting."""

        verbose_name = "arius User Setting"
        verbose_name_plural = "arius User Settings"
        constraints = [
...
class PriceBreak(MetaMixin):
    ...
    quantity = arius.fields.RoundingDecimalField(
        max_digits=15,
        decimal_places=5,
        default=1,
    ...
    price = arius.fields.AriusModelMoneyField(
        max_digits=19,
        decimal_places=6,
        null=True,
...
def get_price(instance, quantity, moq=True, multiples=True, currency=None, break_name: str = 'price_breaks'):
    ...
    if pb_found:
        ...
        return arius.helpers.normalize(cost + instance.base_cost)
    else:
        return None
...
class NewsFeedEntry(models.Model):
    """A NewsFeedEntry represents an entry on the RSS/Atom feed that is generated for arius news.
