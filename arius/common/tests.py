...
from arius.helpers import str2bool
from arius.unit_test import (AriusAPITestCase, AriusTestCase,
                             PluginMixin)
from plugin import registry
from plugin.models import NotificationUserSetting

from .api import WebhookView
from .models import (ColorTheme, AriusSetting, AriusUserSetting,
...
class SettingsTest(AriusTestCase):
    ...
    def test_settings_objects(self):
        ...
        settings = AriusSetting.objects.all()

        self.assertTrue(settings.count() >= 2)

        instance_name = AriusSetting.objects.get(pk=1)
        self.assertEqual(instance_name.key, 'ARIUS_INSTANCE')
        self.assertEqual(instance_name.value, 'My very first arius Instance')

        # Check object lookup (case insensitive)
        self.assertEqual(AriusSetting.get_setting_object('arius_inSTanCE').pk, 1)

    def test_settings_functions(self):
        """Test settings functions and properties."""
        # define settings to check
        instance_ref = 'ARIUS_INSTANCE'
        instance_obj = AriusSetting.get_setting_object(instance_ref, cache=False)

        stale_ref = 'STOCK_STALE_DAYS'
        stale_days = AriusSetting.get_setting_object(stale_ref, cache=False)

        report_size_obj = AriusSetting.get_setting_object('REPORT_DEFAULT_PAGE_SIZE')
        report_test_obj = AriusSetting.get_setting_object('REPORT_ENABLE_TEST_REPORT')
        ...
        # check as_choice
        self.assertEqual(instance_obj.as_choice(), 'My very first arius Instance')
        ...
        self.assertEqual(instance_obj.as_int(), 'arius')  # not an int -> return default
    ...
    def test_allValues(self):
        ...
        # check a few keys
        result = AriusSetting.allValues()
        self.assertIn('ARIUS_INSTANCE', result)
    ...
    def test_setting_data(self):
        ...
        """
        for key, setting in AriusSetting.SETTINGS.items():
        ...
        for key, setting in AriusUserSetting.SETTINGS.items():
            try:
                self.run_settings_check(key, setting)
            except Exception as exc:  # pragma: no cover
    ...
    def test_defaults(self):
        ...
        for key in AriusSetting.SETTINGS.keys():

            value = AriusSetting.get_setting_default(key)

            AriusSetting.set_setting(key, value, self.user)

            self.assertEqual(value, AriusSetting.get_setting(key))

            # Any fields marked as 'boolean' must have a default value specified
            setting = AriusSetting.get_setting_object(key)
    ...
    def test_global_setting_caching(self):
        ...
        cache_key = AriusSetting.create_cache_key(key)
        self.assertEqual(cache_key, 'AriusSetting:PART_NAME_FORMAT')
        ...
        val = AriusSetting.get_setting(key)
        ...
        for val in ['A', '{{ part.IPN }}', 'C']:
            ...
            AriusSetting.set_setting(key, val, None)
            self.assertEqual(cache.get(cache_key).value, val)
            self.assertEqual(AriusSetting.get_setting(key), val)
    ...
    def test_user_setting_caching(self):
        ...
        for user in get_user_model().objects.all():
            setting = AriusUserSetting.get_setting_object(key, user=user)
            cache_key = setting.cache_key
            self.assertEqual(cache_key, f"AriusUserSetting:SEARCH_PREVIEW_RESULTS_user:{user.username}")
            AriusUserSetting.set_setting(key, user.pk, None, user=user)
            self.assertIsNotNone(cache.get(cache_key))

        # Iterate through a second time, ensure the values have been cached correctly
        for user in get_user_model().objects.all():
            value = AriusUserSetting.get_setting(key, user=user)
            self.assertEqual(value, user.pk)


class GlobalSettingsApiTest(AriusAPITestCase):
    ...
    def test_global_settings_api_list(self):
        ...
        for key in AriusSetting.SETTINGS:
            AriusSetting.get_setting_object(key, cache=False)

        response = self.get(url, expected_code=200)

        # Number of results should match the number of settings
        self.assertEqual(len(response.data), len(AriusSetting.SETTINGS.keys()))

    def test_company_name(self):
        """Test a settings object lifecycle e2e."""
        setting = AriusSetting.get_setting_object('ARIUS_COMPANY_NAME')
    ...
    def test_api_detail(self):
        ...
        AriusSetting.objects.filter(key=key).delete()

        # Check that we can access a setting which has not previously been created
        self.assertFalse(AriusSetting.objects.filter(key=key).exists())

        # Access via the API, and the default value should be received
        response = self.get(url, expected_code=200)

        self.assertEqual(response.data['value'], 'arius')
        ...
        setting = AriusSetting.objects.get(key=key)
...
class UserSettingsApiTest(AriusAPITestCase):
    ...
    def test_user_setting_init(self):
        ...
        self.assertFalse(AriusUserSetting.objects.filter(key=key).exists())
        ...
        setting = AriusUserSetting.objects.get(key=key, user=self.user)
    ...
    def test_user_setting_boolean(self):
        ...
        setting = AriusUserSetting.get_setting_object(
...
class NotificationUserSettingsApiTest(AriusAPITestCase):
...
class PluginSettingsApiTest(PluginMixin, AriusAPITestCase):
...
class NotificationTest(AriusAPITestCase):
    ...
    def test_api_list(self):
        ...
        # Ref: https://github.com/ariustechnology/Arius/pull/3154
...
class CommonTest(AriusAPITestCase):
    ...
    def test_restart_flag(self):
        ...
        # set flag true
        common.models.AriusSetting.set_setting('SERVER_RESTART_REQUIRED', False, None)

        # reload the app
        registry.reload_plugins()

        # now it should be false again
        self.assertFalse(common.models.AriusSetting.get_setting('SERVER_RESTART_REQUIRED'))
    ...
    def test_config_api(self):
        ...
        data = [
            ...
            self.get(reverse('api-config-detail', kwargs={'key': 'ARIUS_DEBUG'}), expected_code=200).data,  # detail endpoint
        ]

        for item in data:
            self.assertEqual(item['key'], 'ARIUS_DEBUG')
            self.assertEqual(item['env_var'], 'ARIUS_DEBUG')
...
class CurrencyAPITests(AriusAPITestCase):
...
class NotesImageTest(AriusAPITestCase):
...
class ProjectCodesTest(AriusAPITestCase):
