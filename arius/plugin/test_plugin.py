...
from plugin import AriusPlugin, registry
...
class AriusPluginTests(TestCase):
    """Tests for AriusPlugin."""
    ...
    def setUpTestData(cls):
        ...
        cls.plugin = AriusPlugin()

        class NamedPlugin(AriusPlugin):
            """a named plugin."""
            NAME = 'abc123'

        cls.named_plugin = NamedPlugin()

        class SimpleAriusPlugin(AriusPlugin):
            NAME = 'SimplePlugin'

        cls.plugin_simple = SimpleAriusPlugin()

        class OldAriusPlugin(AriusPlugin):
            PLUGIN_SLUG = 'old'

        cls.plugin_old = OldAriusPlugin()

        class NameAriusPlugin(AriusPlugin):
            NAME = 'Aplugin'
            SLUG = 'a'
            TITLE = 'a titel'
        ...
        cls.plugin_name = NameAriusPlugin()
        cls.plugin_sample = SampleIntegrationPlugin()

        class VersionAriusPlugin(AriusPlugin):
            NAME = 'Version'

            MIN_VERSION = '0.1.0'
            MAX_VERSION = '0.1.3'

        cls.plugin_version = VersionAriusPlugin()
...
class RegistryTests(TestCase):
    ...
    def run_package_test(self, directory):
        ...
        envs = {'ARIUS_PLUGIN_TEST_DIR': directory}
        with mock.patch.dict(os.environ, envs):
            # Reload to redicsover plugins
            registry.reload_plugins(full_reload=True)

            # Depends on the meta set in arius/plugin/mock/simple:SimplePlugin
    ...
    def test_custom_loading(self):
        ...
        # Patch env
        envs = {'ARIUS_PLUGIN_TEST_DIR': 'plugin_test_dir'}
        with mock.patch.dict(os.environ, envs):
    ...
    def test_package_loading(self):
        ...
        subprocess.check_output('pip install arius-zapier'.split())
        ...
        self.assertEqual(plg.name, 'arius_zapier')
