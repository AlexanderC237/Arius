...
class PluginsRegistry:
    ...
    def __init__(self) -> None:
        ...
        """
        # plugin registry
        self.plugins: Dict[str, AriusPlugin] = {}           # List of active instances
        self.plugins_inactive: Dict[str, AriusPlugin] = {}  # List of inactive instances
        self.plugins_full: Dict[str, AriusPlugin] = {}      # List of all plugin instances

        self.plugin_modules: List(AriusPlugin) = []         # Holds all discovered plugins
    ...
    def plugin_dirs(self):
        ...
        if settings.PLUGINS_ENABLED:
            ...
            if settings.TESTING:
                custom_dirs = os.getenv('ARIUS_PLUGIN_TEST_DIR', None)
            else:  # pragma: no cover
                custom_dirs = get_setting('ARIUS_PLUGIN_DIR', 'plugin_dir')
            ...
            if custom_dirs is not None:
                ...
                for pd_text in custom_dirs.split(','):
                    ...
                    if not init_filename.exists():
                        try:
                            init_filename.write_text("# arius plugin directory\n")
                        except Exception:  # pragma: no cover
    ...
    def collect_plugins(self):
        ...
        for plugin in self.plugin_dirs():
            ...
            modules = get_plugins(raw_module, AriusPlugin, path=parent_path)
        ...
        for plg in self.plugin_modules:
            ...
            if settings.PLUGIN_TESTING or builtin or (plg_db and plg_db.active):
                ...
                if (plg_i.MIN_VERSION or plg_i.MAX_VERSION) and not plg_i.check_version():
                    ...
                    p = plg_name
                    v = version.ariusVersion()
                    _msg = _(f"Plugin '{p}' is not compatible with the current arius version {v}")
    ...
    def _clean_registry(self):
        """Remove all plugins from registry."""
        self.plugins: Dict[str, AriusPlugin] = {}
        self.plugins_inactive: Dict[str, AriusPlugin] = {}
        self.plugins_full: Dict[str, AriusPlugin] = {}

    def _update_urls(self):
        from arius.urls import frontendpatterns as urlpattern
        from arius.urls import urlpatterns as global_pattern
        from plugin.urls import get_plugin_urls

        for index, url in enumerate(urlpattern):
            if hasattr(url, 'app_name'):
                if url.app_name == 'admin':
                    urlpattern[index] = re_path(r'^admin/', admin.site.urls, name='arius-admin')
