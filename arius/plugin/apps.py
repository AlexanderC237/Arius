...
The main code for plugin special sauce is in the plugin registry in `arius/plugin/registry.py`.
"""
...
class PluginAppConfig(AppConfig):
    ...
    def ready(self):
        ...
        if not canAppAccessDatabase(allow_test=True, allow_plugins=True):
            ...
            logger.info('Loading arius plugins')

            if not registry.is_loading:
                # this is the first startup
                try:
                    from common.models import AriusSetting
                    if AriusSetting.get_setting('PLUGIN_ON_STARTUP', create=False, cache=False):
