...
class SettingsMixinTest(BaseMixinDefinition, AriusTestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class SettingsCls(SettingsMixin, AriusPlugin):
            SETTINGS = self.TEST_SETTINGS
        self.mixin = SettingsCls()

        class NoSettingsCls(SettingsMixin, AriusPlugin):
            pass
...
class UrlsMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class UrlsCls(UrlsMixin, AriusPlugin):
            def test():
                return 'ccc'
            URLS = [re_path('testpath', test, name='test'), ]
        self.mixin = UrlsCls()

        class NoUrlsCls(UrlsMixin, AriusPlugin):
            pass
...
class AppMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class TestCls(AppMixin, AriusPlugin):
            pass
        self.mixin = TestCls()
...
class NavigationMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class NavigationCls(NavigationMixin, AriusPlugin):
            NAVIGATION = [
        ...
        class NothingNavigationCls(NavigationMixin, AriusPlugin):
            pass
...
class APICallMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        ...
        class MixinCls(APICallMixin, SettingsMixin, AriusPlugin):
            ...
            def get_external_url(self, simple: bool = True):
                ...
                return self.api_call('orgs/arius', simple_response=simple)
        ...
        class WrongCLS(APICallMixin, AriusPlugin):
            pass

        self.mixin_wrong = WrongCLS()

        class WrongCLS2(APICallMixin, AriusPlugin):
    ...
    def test_api_call(self):
        ...
        result = self.mixin.api_call('/orgs/arius', simple_response=False)
        self.assertTrue(result)
        self.assertEqual(result.reason, 'OK')

        # api_call with filter
        result = self.mixin.api_call('repos/arius/arius/stargazers', url_args={'page': '2'})
        self.assertTrue(result)
...
class PanelMixinTests(AriusTestCase):
    ...
    def test_mixin(self):
        ...
        with self.assertRaises(MixinNotImplementedError):
            class Wrong(PanelMixin, AriusPlugin):
                pass
