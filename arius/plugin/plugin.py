...
class VersionMixin:
    ...
    def check_version(self, latest=None) -> bool:
        """Check if plugin functions for the current arius version."""
        from arius import version

        latest = latest if latest else version.ariusVersionTuple()
        min_v = version.ariusVersionTuple(self.MIN_VERSION)
        max_v = version.ariusVersionTuple(self.MAX_VERSION)

        return bool(min_v <= latest <= max_v)


class AriusPlugin(VersionMixin, MixinBase, MetaBase):
    """The AriusPlugin class is used to integrate with 3rd party software.

    DO NOT USE THIS DIRECTLY, USE plugin.AriusPlugin
    """
