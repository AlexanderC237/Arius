...
from arius.config import get_setting
...
def check_daily_holdoff(task_name: str, n_days: int = 1) -> bool:
    ...
    Arguments:
        ...
        setting_name: The name of the global setting, e.g. 'ARIUS_DUMMY_TASK_INTERVAL'
    ...
    from arius.ready import isInTestMode
    ...
    last_success = AriusSetting.get_setting(success_key, '', cache=False)
    ...
    last_attempt = AriusSetting.get_setting(attempt_key, '', cache=False)
...
def record_task_attempt(task_name: str):
    ...
    AriusSetting.set_setting(f'_{task_name}_ATTEMPT', datetime.now().isoformat(), None)
...
def record_task_success(task_name: str):
    ...
    AriusSetting.set_setting(f'_{task_name}_SUCCESS', datetime.now().isoformat(), None)
...
def offload_task(taskname, *args, force_async=False, force_sync=False, **kwargs):
    ...
    try:
        ...
        from arius.status import is_worker_running
    except AppRegistryNotReady:  # pragma: no cover
...
def delete_successful_tasks():
    ...
    try:
        ...
        days = AriusSetting.get_setting('ARIUS_DELETE_TASKS_DAYS', 30)
...
def delete_old_error_logs():
    ...
    try:
        ...
        days = AriusSetting.get_setting('ARIUS_DELETE_ERRORS_DAYS', 30)
        threshold = timezone.now() - timedelta(days=days)
...
def delete_old_notifications():
    ...
    try:
        from common.models import (AriusSetting, NotificationEntry,
                                   NotificationMessage)

        days = AriusSetting.get_setting('ARIUS_DELETE_NOTIFICATIONS_DAYS', 30)
        threshold = timezone.now() - timedelta(days=days)
...
def check_for_updates():
    """Check if there is an update for arius."""
    try:
    ...
    interval = int(common.models.AriusSetting.get_setting('ARIUS_UPDATE_CHECK_INTERVAL', 7, cache=False))

    # Check if we should check for updates *today*
    if not check_daily_holdoff('check_for_updates', interval):
        return

    logger.info("Checking for arius software updates")
    ...
    response = requests.get(
        'https://api.github.com/repos/ariustechnology/arius/releases/latest',
        headers=headers
    )
    ...
    logger.info(f"Latest arius version: '{tag}'")

    # Save the version to the database
    common.models.AriusSetting.set_setting(
        '_ARIUS_LATEST_VERSION',
        tag,
        None
    )
...
def update_exchange_rates():
    ...
    try:
        ...
        from arius.exchange import AriusExchange
    except AppRegistryNotReady:  # pragma: no cover
        # Apps not yet loaded!
    ...
    try:
        ...
        Rate.objects.filter(backend="AriusExchange").exclude(currency__in=currency_codes()).delete()
...
def run_backup():
    ...
    if not AriusSetting.get_setting('ARIUS_BACKUP_ENABLE', False, cache=False):
        # Backups are not enabled - exit early
        return

    interval = int(AriusSetting.get_setting('ARIUS_BACKUP_DAYS', 1, cache=False))
...
def check_for_migrations(worker: bool = True):
    ...
    if not get_setting('ARIUS_AUTO_UPDATE', 'auto_update'):
        return
    ...
    logger.info('Checking arius left maintenance mode')
