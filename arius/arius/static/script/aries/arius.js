...
function ariusDocReady() {
  ...
  $("#offcanvas-notification").on(
    "hidden.bs.offcanvas",
    closeNotificationPanel
  ); // listener for closing the notification panel
}
...
function enableDragAndDrop(elementId, url, options = {}) {
  ...
  if (!element.exists()) {
    console.error(
      `enableDragAndDrop called with invalid target: '${elementId}'`
    );
    return;
  }
  ...
  $(element).on("drop", function (event) {
    ...
    if (isFileTransfer(transfer)) {
      ...
      ariusFormDataUpload(url, formData, {
 ...
 * @returns
 */
function ariusLoad(name, defaultValue) {
  var key = `arius-${name}`;
