...
class MetadataMixin(models.Model):
    ...
    - The internal arius business logic will make no use of this field
...
class ReferenceIndexingMixin(models.Model):
    ...
    def get_reference_pattern(cls):
        ...
        return AriusSetting.get_setting(cls.REFERENCE_PATTERN_SETTING, create=False).strip()
    ...
    def get_next_reference(cls):
        ...
        try:
            reference = arius.format.extract_named_group('ref', reference, cls.get_reference_pattern())
        except Exception:
        ...
        incremented = arius.helpers.increment(reference)
            ...
            try:
                ...
                if ref in attempts:
                    ...
                    if cls.objects.filter(reference=ref).exists():
                        ...
                        ctx['ref'] = arius.helpers.increment(ctx['ref'])
                    else:
    ...
    def validate_reference_pattern(cls, pattern):
        ...
        try:
            info = arius.format.parse_format_string(pattern)
    ...
    def validate_reference_field(cls, value):
        ...
        if not arius.format.validate_string(value, pattern):
...
class AriusAttachment(models.Model):
    ...
    def getSubdir(self):
        ...
        Note: Re-implement this for each subclass of AriusAttachment
        """
        return "attachments"
    ...
    link = arius.fields.AriusURLField(
        blank=True, null=True,
        verbose_name=_('Link'),
...
class AriusTree(MPTTModel):
    ...
    def api_instance_filters(self):
        """Instance filters for AriusTree models."""
        return {
            'parent': {
    ...
    def save(self, *args, **kwargs):
        """Custom save method for AriusTree abstract model"""
        ...
        pathstring = arius.helpers.constructPathString(
...
class AriusNotesMixin(models.Model):
    ...
    notes = arius.fields.AriusNotesField(
        verbose_name=_('Notes'),
        help_text=_('Markdown notes (optional)'),
    )


class AriusBarcodeMixin(models.Model):
    ...
    def format_barcode(self, **kwargs):
        ...
        return arius.helpers.MakeBarcode(
    ...
    def assign_barcode(self, barcode_hash=None, barcode_data=None, raise_error=True, save=True):
        ...
        if barcode_hash is None:
            barcode_hash = arius.helpers.hash_barcode(barcode_data)
...
@receiver(pre_delete, sender=AriusTree, dispatch_uid='tree_pre_delete_log')
def before_delete_tree_item(sender, instance, using, **kwargs):
    """Receives pre_delete signal from AriusTree object.
...
def after_error_logged(sender, instance: Error, created: bool, **kwargs):
    ...
    if created:
        try:
            ...
            link = arius.helpers_model.construct_absolute_url(
