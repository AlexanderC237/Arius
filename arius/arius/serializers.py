"""Serializers used in various arius apps."""
...
from arius.fields import AriusRestURLField, AriusURLField
from arius.helpers_model import download_image_from_url


class AriusMoneySerializer(MoneyField):
...
class AriusCurrencySerializer(serializers.ChoiceField):
...
class AriusModelSerializer(serializers.ModelSerializer):
    """Inherits the standard Django ModelSerializer class, but also ensures that the underlying model class data are checked on validation."""

    # Switch out URLField mapping
    serializer_field_mapping = {
        **serializers.ModelSerializer.serializer_field_mapping,
        models.URLField: AriusRestURLField,
        AriusURLField: AriusRestURLField,
    }
...
class AriusTaggitSerializer(TaggitSerializer):
...
class AriusTagModelSerializer(AriusTaggitSerializer, AriusModelSerializer):
    """Combination of AriusTaggitSerializer and AriusModelSerializer."""
    pass


class UserSerializer(AriusModelSerializer):
...
class AriusAttachmentSerializerField(serializers.FileField):
...
class AriusAttachmentSerializer(AriusModelSerializer):
    """Special case of an AriusModelSerializer, which handles an "attachment" model.
    ...
    attachment = AriusAttachmentSerializerField(
        required=False,
        allow_null=False,
    )
...
class AriusImageSerializerField(serializers.ImageField):
...
class AriusDecimalField(serializers.FloatField):
...
class RemoteImageMixin(metaclass=serializers.SerializerMetaclass):
    ...
    def validate_remote_image(self, url):
        ...
        if not common_models.AriusSetting.get_setting('ARIUS_DOWNLOAD_FROM_URL'):
