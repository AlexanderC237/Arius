...
from arius.urls import frontendpatterns
...
class AuthRequiredMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            However when running an external application (e.g. the arius app or Python library),
    ...
    def require_2fa(self, request):
        ...
        try:
            if url_matcher.resolve(request.path[1:]):
                return AriusSetting.get_setting('LOGIN_ENFORCE_MFA')
        except Resolver404:
            pass
        return False
...
class AriusRemoteUserMiddleware(PersistentRemoteUserMiddleware):
...
class AriusExceptionProcessor(ExceptionProcessor):
