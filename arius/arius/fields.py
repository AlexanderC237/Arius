...
class AriusRestURLField(RestURLField):
...
class AriusURLField(models.URLField):
    """Custom URL field which has custom scheme validators."""

    default_validators = [AllowedURLValidator()]

    def __init__(self, **kwargs):
        """Initialization method for AriusURLField"""

        # Max length for AriusURLField is set to 200
        kwargs['max_length'] = 200
        super().__init__(**kwargs)
...
class AriusModelMoneyField(ModelMoneyField):
    ...
    def formfield(self, **kwargs):
        ...
        kwargs['form_class'] = AriusMoneyField
...
class AriusMoneyField(MoneyField):
    """Custom MoneyField for clean migrations while using dynamic currency settings."""
...
def round_decimal(value, places, normalize=False):
    ...
    if type(value) in [Decimal, float]:
        ...
        if normalize:
            ...
            value = arius.helpers.normalize(value)
...
class AriusNotesField(models.TextField):
