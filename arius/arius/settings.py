"""Django settings for arius project.
...
from arius.config import get_boolean_setting, get_custom_file, get_setting
from arius.sentry import default_sentry_dsn, init_sentry
from arius.version import ariusApiVersion

from . import config

ARIUS_NEWS_URL = 'https://arius.org/news/feed.atom'
...
DEBUG = get_boolean_setting('ARIUS_DEBUG', 'debug', True)

# Configure logging settings
log_level = get_setting('ARIUS_LOG_LEVEL', 'log_level', 'WARNING')
...
DBBACKUP_STORAGE = get_setting(
    'ARIUS_BACKUP_STORAGE',
    'backup_storage',
    'django.core.files.storage.FileSystemStorage'
)

# Default backup configuration
DBBACKUP_STORAGE_OPTIONS = get_setting('ARIUS_BACKUP_OPTIONS', 'backup_options', None)
...
INSTALLED_APPS = [
    ...
    'generic',
    'arius.apps.AriusConfig',       # arius app runs last
...
MIDDLEWARE = CONFIG.get('middleware', [
    ...
    'arius.middleware.AriusRemoteUserMiddleware',       # Remote / proxy auth
    'django_otp.middleware.OTPMiddleware',                      # MFA support
    'arius.middleware.CustomAllauthTwoFactorMiddleware',    # Flow control for allauth
    'django.contrib.messages.middleware.MessageMiddleware',
    'django.middleware.clickjacking.XFrameOptionsMiddleware',
    'arius.middleware.AuthRequiredMiddleware',
    'arius.middleware.Check2FAMiddleware',                  # Check if the user should be forced to use MFA
    'maintenance_mode.middleware.MaintenanceModeMiddleware',
    'arius.middleware.AriusExceptionProcessor',         # Error reporting
])
...
DEBUG_TOOLBAR_ENABLED = DEBUG and get_setting('ARIUS_DEBUG_TOOLBAR', 'debug_toolbar', False)
...
DOCKER = get_boolean_setting('ARIUS_DOCKER', default_value=False)
...
# arius URL configuration

# Base URL for admin pages (default="admin")
ARIUS_ADMIN_URL = get_setting(
    'ARIUS_ADMIN_URL',
    config_key='admin_url',
    default_value='admin'
)

ROOT_URLCONF = 'arius.urls'
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                # Custom arius context processors
                'arius.context.health_status',
                'arius.context.status_codes',
                'arius.context.user_roles',
            ],
            'loaders': [(
                'arius.template.AriusTemplateLoader', [
...
REST_FRAMEWORK = {
    'EXCEPTION_HANDLER': 'arius.exceptions.exception_handler',
    ...
    'DEFAULT_PERMISSION_CLASSES': (
        ...
        'arius.permissions.RolePermission',
    ),
    'DEFAULT_SCHEMA_CLASS': 'drf_spectacular.openapi.AutoSchema',
    'DEFAULT_METADATA_CLASS': 'arius.metadata.AriusMetadata',
...
# dj-rest-auth
# JWT switch
USE_JWT = get_boolean_setting('ARIUS_USE_JWT', 'use_jwt', False)
REST_USE_JWT = USE_JWT
OLD_PASSWORD_FIELD_ENABLED = True
REST_AUTH_REGISTER_SERIALIZERS = {'REGISTER_SERIALIZER': 'arius.forms.CustomRegisterSerializer'}

# JWT settings - rest_framework_simplejwt
if USE_JWT:
    JWT_AUTH_COOKIE = 'arius-auth'
    JWT_AUTH_REFRESH_COOKIE = 'arius-token'
...
SPECTACULAR_SETTINGS = {
    'TITLE': 'arius API',
    'DESCRIPTION': 'API for arius - the intuitive open source inventory management system',
    'LICENSE': {'MIT': 'https://github.com/ariustechnology/Arius/blob/master/LICENSE'},
    'EXTERNAL_DOCS': {'docs': 'https://docs.arius.org', 'web': 'https://arius.org'},
    'VERSION': ariusApiVersion(),
    'SERVE_INCLUDE_SCHEMA': False,
}

WSGI_APPLICATION = 'arius.wsgi.application'
...
if "postgres" in db_engine:  # pragma: no cover
    ...
    if "connect_timeout" not in db_options:
        ...
        db_options["connect_timeout"] = int(
            get_setting('ARIUS_DB_TIMEOUT', 'database.timeout', 2)
        )
    ...
    if "keepalives" not in db_options:
        db_options["keepalives"] = int(
            get_setting('ARIUS_DB_TCP_KEEPALIVES', 'database.tcp_keepalives', 1)
        )

    # Seconds after connection is idle to send keep alive
    if "keepalives_idle" not in db_options:
        db_options["keepalives_idle"] = int(
            get_setting('ARIUS_DB_TCP_KEEPALIVES_IDLE', 'database.tcp_keepalives_idle', 1)
        )
    ...
    if "keepalives_interval" not in db_options:
        db_options["keepalives_interval"] = int(
            get_setting("ARIUS_DB_TCP_KEEPALIVES_INTERVAL", "database.tcp_keepalives_internal", "1")
        )
    ...
    if "keepalives_count" not in db_options:
        db_options["keepalives_count"] = int(
            get_setting("ARIUS_DB_TCP_KEEPALIVES_COUNT", "database.tcp_keepalives_count", "5")
        )
    ...
    if "isolation_level" not in db_options:
        serializable = get_boolean_setting('ARIUS_DB_ISOLATION_SERIALIZABLE', 'database.serializable', False)
...
# login settings
REMOTE_LOGIN = get_boolean_setting('ARIUS_REMOTE_LOGIN', 'remote_login_enabled', False)
REMOTE_LOGIN_HEADER = get_setting('ARIUS_REMOTE_LOGIN_HEADER', 'remote_login_header', 'REMOTE_USER')

# sentry.io integration for error reporting
SENTRY_ENABLED = get_boolean_setting('ARIUS_SENTRY_ENABLED', 'sentry_enabled', False)

# Default Sentry DSN (can be overridden if user wants custom sentry integration)
SENTRY_DSN = get_setting('ARIUS_SENTRY_DSN', 'sentry_dsn', default_sentry_dsn())
SENTRY_SAMPLE_RATE = float(get_setting('ARIUS_SENTRY_SAMPLE_RATE', 'sentry_sample_rate', 0.1))
...
if SENTRY_ENABLED and SENTRY_DSN:  # pragma: no cover
    ...
    init_sentry(SENTRY_DSN, SENTRY_SAMPLE_RATE, arius_tags)

# Cache configuration
cache_host = get_setting('ARIUS_CACHE_HOST', 'cache.host', None)
cache_port = get_setting('ARIUS_CACHE_PORT', 'cache.port', '6379', typecast=int)
...
_q_worker_timeout = int(get_setting('ARIUS_BACKGROUND_TIMEOUT', 'background.timeout', 90))

# django-q background worker configuration
Q_CLUSTER = {
    'name': 'arius',
    'label': 'Background Tasks',
    'workers': int(get_setting('ARIUS_BACKGROUND_WORKERS', 'background.workers', 4)),
    'timeout': _q_worker_timeout,
    'retry': min(120, _q_worker_timeout + 30),
    'max_attempts': int(get_setting('ARIUS_BACKGROUND_MAX_ATTEMPTS', 'background.max_attempts', 5)),
    'queue_limit': 50,
    'catch_up': False,
    'bulk': 10,
    'orm': 'default',
...
LOGOUT_REDIRECT_URL = get_setting('ARIUS_LOGOUT_REDIRECT_URL', 'logout_redirect_url', 'index')
...
EXTRA_URL_SCHEMES = get_setting('ARIUS_EXTRA_URL_SCHEMES', 'extra_url_schemes', [])
...
LANGUAGE_CODE = get_setting('ARIUS_LANGUAGE', 'language', 'en-us')
...
EXCHANGE_BACKEND = 'arius.exchange.AriusExchange'

# Email configuration options
EMAIL_BACKEND = get_setting('ARIUS_EMAIL_BACKEND', 'email.backend', 'django.core.mail.backends.smtp.EmailBackend')
EMAIL_HOST = get_setting('ARIUS_EMAIL_HOST', 'email.host', '')
EMAIL_PORT = get_setting('ARIUS_EMAIL_PORT', 'email.port', 25, typecast=int)
EMAIL_HOST_USER = get_setting('ARIUS_EMAIL_USERNAME', 'email.username', '')
EMAIL_HOST_PASSWORD = get_setting('ARIUS_EMAIL_PASSWORD', 'email.password', '')
EMAIL_SUBJECT_PREFIX = get_setting('ARIUS_EMAIL_PREFIX', 'email.prefix', '[arius] ')
EMAIL_USE_TLS = get_boolean_setting('ARIUS_EMAIL_TLS', 'email.tls', False)
EMAIL_USE_SSL = get_boolean_setting('ARIUS_EMAIL_SSL', 'email.ssl', False)

DEFAULT_FROM_EMAIL = get_setting('ARIUS_EMAIL_SENDER', 'email.sender', '')
...
TIME_ZONE = get_setting('ARIUS_TIMEZONE', 'timezone', 'UTC')
...
SOCIAL_BACKENDS = get_setting('ARIUS_SOCIAL_BACKENDS', 'social_backends', [], typecast=list)

for app in SOCIAL_BACKENDS:
    INSTALLED_APPS.append(app)  # pragma: no cover

SOCIALACCOUNT_PROVIDERS = get_setting('ARIUS_SOCIAL_PROVIDERS', 'social_providers', None, typecast=dict)

SOCIALACCOUNT_STORE_TOKENS = True

# settings for allauth
ACCOUNT_EMAIL_CONFIRMATION_EXPIRE_DAYS = get_setting('ARIUS_LOGIN_CONFIRM_DAYS', 'login_confirm_days', 3, typecast=int)
ACCOUNT_LOGIN_ATTEMPTS_LIMIT = get_setting('ARIUS_LOGIN_ATTEMPTS', 'login_attempts', 5, typecast=int)
ACCOUNT_DEFAULT_HTTP_PROTOCOL = get_setting('ARIUS_LOGIN_DEFAULT_HTTP_PROTOCOL', 'login_default_protocol', 'http')
...
ACCOUNT_FORMS = {
    ...
    'signup': 'arius.forms.CustomSignupForm',
...
SOCIALACCOUNT_ADAPTER = 'arius.forms.CustomSocialAccountAdapter'
ACCOUNT_ADAPTER = 'arius.forms.CustomAccountAdapter'
...
# Are plugins enabled?
PLUGINS_ENABLED = get_boolean_setting('ARIUS_PLUGINS_ENABLED', 'plugins_enabled', False)

PLUGIN_FILE = config.get_plugin_file()

# Plugin test settings
PLUGIN_TESTING = get_setting('ARIUS_PLUGIN_TESTING', 'PLUGIN_TESTING', TESTING)                     # Are plugins being tested?
PLUGIN_TESTING_SETUP = get_setting('ARIUS_PLUGIN_TESTING_SETUP', 'PLUGIN_TESTING_SETUP', False)     # Load plugins from setup hooks in testing?
PLUGIN_TESTING_EVENTS = False                                                                           # Flag if events are tested right now
PLUGIN_RETRY = get_setting('ARIUS_PLUGIN_RETRY', 'PLUGIN_RETRY', 5)                                 # How often should plugin loading be tried?
PLUGIN_FILE_CHECKED = False                                                                             # Was the plugin file checked?

# Site URL can be specified statically, or via a run-time setting
SITE_URL = get_setting('ARIUS_SITE_URL', 'site_url', None)
...
CUSTOM_LOGO = get_custom_file('ARIUS_CUSTOM_LOGO', 'customize.logo', 'custom logo', lookup_media=True)
CUSTOM_SPLASH = get_custom_file('ARIUS_CUSTOM_SPLASH', 'customize.splash', 'custom splash')

CUSTOMIZE = get_setting('ARIUS_CUSTOMIZE', 'customize', {})
if DEBUG:
    logger.info("arius running with DEBUG enabled")
...
CUSTOM_FLAGS = get_setting('ARIUS_FLAGS', 'flags', None, typecast=dict)
if CUSTOM_FLAGS:
