...
from arius.sanitizer import sanitize_svg
...
class ConversionTest(TestCase):
    ...
    def test_dimensionless_units(self):
        ...
        for val, expected in tests.items():
            q = arius.conversion.convert_physical_value(val).to_base_units()
...
class FormatTest(TestCase):
    ...
    def test_parse(self):
        ...
        info = arius.format.parse_format_string(fmt)
        ...
        for fmt in [
            ...
            with self.assertRaises(ValueError):
                arius.format.parse_format_string(fmt)
    ...
    def test_create_regex(self):
        ...
        for fmt, reg in tests.items():
            self.assertEqual(arius.format.construct_format_regex(fmt), reg)
    ...
    def test_validate_format(self):
        ...
        for value, pattern in {
            ...
            self.assertTrue(arius.format.validate_string(value, pattern))
        ...
        for value, pattern in {
            ...
            self.assertFalse(arius.format.validate_string(value, pattern))
    ...
    def test_extract_value(self):
        ...
        for k, v in tests.items():
            self.assertEqual(arius.format.extract_named_group('ref', v, fmt), k)
        ...
        for v in tests.values():
            with self.assertRaises(ValueError):
                arius.format.extract_named_group('ref', v, fmt)
        ...
        for k, v in data.items():
            self.assertEqual(arius.format.extract_named_group(k, val, fmt), v)

        # Test for error conditions

        # Raises a ValueError as the format string is bad
        with self.assertRaises(ValueError):
            arius.format.extract_named_group(
                "test",
                "PO-1234-5",
                "PO-{test}-{"
            )
...
class TestHelpers(TestCase):
    """Tests for arius helper functions."""
    ...
    def test_logo_image(self):
        ...
        self.assertEqual(logo, '/static/img/arius.png')

        logo = helpers.getLogoImage(as_file=True)
        self.assertEqual(logo, f'file://{settings.STATIC_ROOT}/img/arius.png')
    ...
    def test_download_image(self):
        ...
        for url in [
            ...
            with self.assertRaises(django_exceptions.ValidationError):
                arius.helpers_model.download_image_from_url(url)
        ...
        def dl_helper(url, expected_error, timeout=2.5, retries=3):
            ...
            with self.assertRaises(expected_error):
                while tries < retries:
                    ...
                    try:
                        arius.helpers_model.download_image_from_url(url, timeout=timeout)
                        break
        ...
        large_img = "https://github.com/ariustechnology/Arius/raw/master/Arius/Arius/static/img/paper_splash_large.jpg"

        AriusSetting.set_setting('ARIUS_DOWNLOAD_IMAGE_MAX_SIZE', 1, change_user=None)

        # Attempt to download an image which is too large
        with self.assertRaises(ValueError):
            arius.helpers_model.download_image_from_url(large_img, timeout=10)

        # Increase allowable download size
        AriusSetting.set_setting('ARIUS_DOWNLOAD_IMAGE_MAX_SIZE', 5, change_user=None)
    ...
    def test_model_mixin(self):
        ...
        from arius.models import AriusBarcodeMixin

        models = arius.helpers_model.getModelsWithMixin(AriusBarcodeMixin)
        ...
        self.assertNotIn(AriusSetting, models)
...
class TestVersionNumber(TestCase):
    ...
    def test_tuple(self):
        """Test ariusVersionTuple."""
        v = version.ariusVersionTuple()
        self.assertEqual(len(v), 3)

        s = '.'.join([str(i) for i in v])

        self.assertTrue(s in version.ariusVersion())

    def test_comparison(self):
        """Test direct comparison of version numbers."""
        v_a = version.ariusVersionTuple('1.2.0')
        v_b = version.ariusVersionTuple('1.2.3')
        v_c = version.ariusVersionTuple('1.2.4')
        v_d = version.ariusVersionTuple('2.0.0')
    ...
    def test_commit_info(self):
        ...
        envs = {
            'ARIUS_COMMIT_HASH': 'abcdef',
            'ARIUS_COMMIT_DATE': '2022-12-31'
        }

        # Check that the environment variables take priority

        with mock.patch.dict(os.environ, envs):
            self.assertEqual(version.ariusCommitHash(), 'abcdef')
            self.assertEqual(version.ariusCommitDate(), '2022-12-31')
        ...
        self.assertEqual(hash, version.ariusCommitHash())

        d = str(subprocess.check_output('git show -s --format=%ci'.split()), 'utf-8').strip().split(' ')[0]
        self.assertEqual(d, version.ariusCommitDate())
...
class CurrencyTests(TestCase):
    ...
    def test_rates(self):
        ...
        for _ in range(10):
            arius.tasks.update_exchange_rates()
...
class TestSettings(AriusTestCase):
    ...
    def run_reload(self, envs=None):
        """Helper function to reload arius."""
        # Set default - see B006
        if envs is None:
            envs = {}
    ...
    def test_set_user_to_few(self):
        ...
        # not enough set
        self.run_reload({
            'ARIUS_ADMIN_USER': 'admin'
        })
        self.assertEqual(user_count(), 1)

        # enough set
        self.run_reload({
            'ARIUS_ADMIN_USER': 'admin',  # set username
            'ARIUS_ADMIN_EMAIL': 'info@example.com',  # set email
            'ARIUS_ADMIN_PASSWORD': 'password123'  # set password
        })
        ...
        self.run_reload({
            'ARIUS_ADMIN_USER': username2,
            'ARIUS_ADMIN_EMAIL': email2,
            'ARIUS_ADMIN_PASSWORD': password2,
        })
        self.assertEqual(user_count(), 3)
    ...
    def test_initial_install(self):
        ...
        if not settings.DOCKER:
            ...
            AriusSetting.set_setting('PLUGIN_ON_STARTUP', True, self.user)
    ...
    def test_helpers_cfg_file(self):
        ...
        valid = [
            'arius/config.yaml',
            'arius/data/config.yaml',
        ]

        self.assertTrue(any(opt in str(config.get_config_file()).lower() for opt in valid))

        # with env set
        with self.in_env_context({'ARIUS_CONFIG_FILE': 'my_special_conf.yaml'}):
            self.assertIn('arius/my_special_conf.yaml', str(config.get_config_file()).lower())
    ...
    def test_helpers_plugin_file(self):
        ...
        valid = [
            'arius/plugins.txt',
            'arius/data/plugins.txt',
        ]

        self.assertTrue(any(opt in str(config.get_plugin_file()).lower() for opt in valid))

        # with env set
        with self.in_env_context({'ARIUS_PLUGIN_FILE': 'my_special_plugins.txt'}):
...
class TestInstanceName(AriusTestCase):
    """Unit tests for instance name."""

    def test_instance_name(self):
        """Test instance name settings."""
        # default setting
        self.assertEqual(version.ariusInstanceTitle(), 'arius')

        # set up required setting
        AriusSetting.set_setting("ARIUS_INSTANCE_TITLE", True, self.user)
        AriusSetting.set_setting("ARIUS_INSTANCE", "Testing title", self.user)

        self.assertEqual(version.ariusInstanceTitle(), 'Testing title')
    ...
    def test_instance_url(self):
        ...
        # Set up required setting
        AriusSetting.set_setting("ARIUS_BASE_URL", "http://127.1.2.3", self.user)
...
class TestOffloadTask(AriusTestCase):
    ...
    def test_offload_tasks(self):
        ...
        Ref: https://github.com/ariustechnology/Arius/pull/3273
        """
    ...
    def test_daily_holdoff(self):
        ...
        with self.assertLogs(logger='arius', level='INFO') as cm:
            # With a non-positive interval, task will not run
            result = arius.tasks.check_daily_holdoff('some_task', 0)
            self.assertFalse(result)
            self.assertIn('Specified interval', str(cm.output))

        with self.assertLogs(logger='arius', level='INFO') as cm:
            # First call should run without issue
            result = arius.tasks.check_daily_holdoff('dummy_task')
        ...
        AriusSetting.set_setting('_dummy_task_ATTEMPT', t_old, None)

        result = arius.tasks.check_daily_holdoff('dummy_task', 5)
        self.assertTrue(result)

        # Last attempt should have been updated
        self.assertNotEqual(t_old, AriusSetting.get_setting('_dummy_task_ATTEMPT', '', cache=False))
        ...
        AriusSetting.set_setting('_dummy_task_ATTEMPT', t_old, None)
        AriusSetting.set_setting('_dummy_task_SUCCESS', t_old, None)

        with self.assertLogs(logger='arius', level='INFO') as cm:
            result = arius.tasks.check_daily_holdoff('dummy_task', 7)
            self.assertFalse(result)
            self.assertIn('Last successful run for', str(cm.output))

            result = arius.tasks.check_daily_holdoff('dummy_task', 2)
            self.assertTrue(result)


class BarcodeMixinTest(AriusTestCase):
    """Tests for the AriusBarcodeMixin mixin class"""
    ...
    def test_bacode_hash(self):
        ...
        for barcode, hash in hashing_tests.items():
            self.assertEqual(arius.helpers.hash_barcode(barcode), hash)
