...
def health_status(request):
    ...
    if hasattr(request, '_arius_health_status'):
        # Do not duplicate efforts
        return {}

    request._arius_health_status = True

    status = {
        'django_q_running': arius.status.is_worker_running(),
        'email_configured': arius.email.is_email_configured(),
    }
    ...
    status['up_to_date'] = arius.version.isAriusUpToDate()
...
def status_codes(request):
    ...
    if hasattr(request, '_arius_status_codes'):
        # Do not duplicate efforts
        return {}

    request._arius_status_codes = True
