"""Provides helper functions used throughout the arius project."""
...
def getLogoImage(as_file=False, custom=True):
    """Return the arius logo image, or a custom logo if available."""
    ...
    if as_file:
        path = settings.STATIC_ROOT.joinpath('img/arius.png')
        return f"file://{path}"
    else:
        return getStaticUrl('img/arius.png')


def getSplashScren(custom=True):
    """Return the arius splash screen, or a custom splash if available"""
    ...
    return static_storage.url("img/arius_splash.jpg")
...
def MakeBarcode(cls_name, object_pk: int, object_data=None, **kwargs):
    """Generate a string for a barcode. Adds some global arius parameters.
    ...
    if brief:
        ...
        data['tool'] = 'arius'
        data['version'] = arius.version.ariusVersion()
        data['instance'] = arius.version.ariusInstanceName()
