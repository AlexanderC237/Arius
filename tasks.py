"""Tasks for automating certain actions and interacting with arius from the CLI."""
...
def managePyDir():
    ...
    return localDir().joinpath('arius')
...
def plugins(c):
    ...
    from arius.arius.config import get_plugin_file
...
def translate_stats(c):
    ...
    path = Path('arius', 'script', 'translation_stats.py')
...
def translate(c):
    ...
    it is performed as part of the arius translation toolchain.
    """
...
def backup(c):
    ...
    print("Backing up arius database...")
    manage(c, "dbbackup --noinput --clean --compress")
    print("Backing up arius media files...")
...
def restore(c):
    ...
    print("Restoring arius database...")
    manage(c, "dbrestore --noinput --uncompress")
    print("Restoring arius media files...")
...
def migrate(c):
    ...
    """
    print("Running arius database migrations...")
    print("========================================")
    ...
    print("========================================")
    print("arius database migrations completed!")
...
def delete_data(c, force=False):
    ...
    """
    print("Deleting all data from arius database...")
...
@task(pre=[wait])
def worker(c):
    """Run the arius background worker process."""
...
def render_js_files(c):
    ...
    manage(c, "test arius.ci_render_js")
...
def test_translations(c):
    ...
    # setup django
    base_path = Path.cwd()
    new_base_path = pathlib.Path('arius').resolve()
    sys.path.append(str(new_base_path))
    os.chdir(new_base_path)
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'arius.settings')
    django.setup()
...
def test(c, disable_pty=False, runtest='', migrations=False, report=False, coverage=False):
    """Run unit-tests for arius codebase.
...
def setup_test(c, ignore_update=False, dev=False, path="arius-demo-dataset"):
    """Setup a testing environment."""

    from arius.arius.config import get_media_dir
    ...
    c.run(f'git clone https://github.com/ariustechnology/demo-dataset {path} -v --depth=1')
    print("========================================")
    ...
    import_records(c, filename=f'{path}/arius_data.json', clear=True)
...
@task(default=True)
def version(c):
    """Show the current version of arius."""
    import arius.arius.version as AriusVersion
    from arius.arius.config import (get_config_file, get_media_dir,
                                    get_static_dir)
...
Django      {AriusVersion.ariusDjangoVersion()}
arius   {AriusVersion.ariusVersion()}
API         {AriusVersion.ariusApiVersion()}

Commit hash:{AriusVersion.ariusCommitHash()}
Commit date:{AriusVersion.ariusCommitDate()}""")
    if len(sys.argv) == 1 and sys.argv[0].startswith('/opt/arius/env/lib/python'):
        print("""
